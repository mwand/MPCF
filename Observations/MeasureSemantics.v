...
Definition distvalM :
  (den_typeM (Stype Real_type))
    -> (den_typeM (Meastype Real_type)) 
  := mu_1.
...
Definition obsvalM : (den_typeM (Stype Real_type)) ->
                     (den_typeM (Stype Real_type)) ->
                     (den_typeM (Meastype Empty_type))
  := fun r x =>
       fun (k : unit -> real) =>
         (times (density_1 r x) (k tt)).
...
Function den_valM {G o} (val : Val G o) (r : (@den_env den_typeM G))
  ...
  :=
    ...
    | prodval v1 v2 => (den_valM v1 r, den_valM v2 r)
      ...
      | obsval p obs => obsvalM (den_valM p r) (den_valM obs r)
